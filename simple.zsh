...
prompt_simple_return_status(){
  local status exit_status=$1 var=$2
  SETCOLOR_DEFAULT=$prompt_simple_colors[status:default]
  for s in $(echo -en "${exit_status}"); do
    if [ "${s}" -eq 147 ] ; then
      SETCOLOR=${SETCOLOR_DEFAULT}
      break
    elif [ "${s}" -gt 100 ] ; then
      SETCOLOR=$prompt_simple_colors[status:error]
      break
    elif [ "${s}" -gt 0 ] ; then
      SETCOLOR=$prompt_simple_colors[status:continuation]
    fi
  done
  if [ "${SETCOLOR}" != "${SETCOLOR_DEFAULT}" ]; then
    status+=("%F{$SETCOLOR}""${exit_status// /|}")
  else
    status+=("%F{$SETCOLOR}")
  fi

  typeset -g "${var}" = "${status}"
}

prompt_simple_cmd_return_status() {
  integer exit_status
  exit_status=${pipestatus[*]}
  typeset -g prompt_simple_cmd_return_status=
  ((${SIMPLE_CMD_ENABLE_STATUS:-true})) && {
    prompt_simple_return_status $exit_status "prompt_simple_cmd_return_status"
  }
}
...
prompt_simple_prompt_prerender() {
  ...
  # Path
  local prompt_path=('%F{${prompt_simple_colors[path]}}%c%f')

  # If a virtualenv is activated, display it in grey and show it under username
  [[ -n $VIRTUAL_ENV ]] && prompt_parts=('%(10V.%F{$prompt_simple_colors[virtualenv]}%10v%f.)%f'${prompt_simple_state[accent]}${prompt_path})
  ...
  if [[ -n $prompt_simple_vcs_info[branch] ]]; then
    git_parts+=("%F{$git_color}"'${prompt_simple_vcs_info[branch]}'"%F{$git_dirty_color}"'${prompt_simple_git_dirty}%f'${prompt_simple_state[accent]})
  fi
  ...
  [[ -n $prompt_simple_cmd_return_status ]] && rprompt_parts+=('${prompt_simple_cmd_return_status}%f')

  # Contruct new prompt after cleaning
  local -ah ps1
  local -ah rps1
  ps1=(${(j..)prompt_parts}$prompt_path$prompt_indicator)
...
prompt_simple_precmd() {
  ...
  if [[ -n $ZSH_THEME ]]; then
    ...
    print "For more information, see: https://github.com/aarnphm/simple"
...
prompt_simple_async_callback() {
  ...
  [[ ${prompt_simple_async_render_requested:-$do_render} = 1 ]] && prompt_simple_prompt_prerender
  unset prompt_simple_async_render_requested
}
...
prompt_simple_setup() {
    ...
    prompt_simple_colors_default=(
      ...
      host                 242
      path                 blue
      status:default       green
      status:error         167
      status:continuation  214
      prompt:error         red
